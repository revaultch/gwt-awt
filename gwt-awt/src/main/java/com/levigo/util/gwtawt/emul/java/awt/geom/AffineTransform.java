/*
 * Copyright (c) 1996, 2010, Oracle and/or its affiliates. All rights reserved. DO NOT ALTER OR
...
/**
 * The <code>AffineTransform</code> class represents a 2D affine transform that performs a linear
 * mapping from 2D coordinates to other 2D coordinates that preserves the "straightness" and
 * "parallelness" of lines. Affine transformations can be constructed using sequences of
 * translations, scales, flips, rotations, and shears.
 * <p>
 * Such a coordinate transformation can be represented by a 3 row by 3 column matrix with an implied
 * last row of [ 0 0 1 ]. This matrix transforms source coordinates {@code (x,y)} into destination
 * coordinates {@code (x',y')} by considering them to be a column vector and multiplying the
 ...
 * <p>
 * In some variations of the <code>rotate</code> methods in the <code>AffineTransform</code> class,
 * a double-precision argument specifies the angle of rotation in radians. These methods have
 * special handling for rotations of approximately 90 degrees (including multiples such as 180, 270,
 * and 360 degrees), so that the common case of quadrant rotation is handled more efficiently. This
 * special handling can cause angles very close to multiples of 90 degrees to be treated as if they
 * were exact multiples of 90 degrees. For small multiples of 90 degrees the range of angles treated
 * as a quadrant rotation is approximately 0.00000121 degrees wide. This section explains why such
 * special care is needed and how it is implemented.
 * <p>
 * Since 90 degrees is represented as <code>PI/2</code> in radians, and since PI is a transcendental
 * (and therefore irrational) number, it is not possible to exactly represent a multiple of 90
 * degrees as an exact double precision value measured in radians. As a result it is theoretically
 * impossible to describe quadrant rotations (90, 180, 270 or 360 degrees) using these values.
 * Double precision floating point values can get very close to non-zero multiples of
 * <code>PI/2</code> but never close enough for the sine or cosine to be exactly 0.0, 1.0 or -1.0.
 * The implementations of <code>Math.sin()</code> and <code>Math.cos()</code> correspondingly never
 * return 0.0 for any case other than <code>Math.sin(0.0)</code>. These same implementations do,
 * however, return exactly 1.0 and -1.0 for some range of numbers around each multiple of 90 degrees
 * since the correct answer is so close to 1.0 or -1.0 that the double precision significand cannot
 * represent the difference as accurately as it can for numbers that are near 0.0.
 * <p>
 * The net result of these issues is that if the <code>Math.sin()</code> and <code>Math.cos()</code>
 * methods are used to directly generate the values for the matrix modifications during these
 * radian-based rotation operations then the resulting transform is never strictly classifiable as a
 * quadrant rotation even for a simple case like <code>rotate(Math.PI/2.0)</code>, due to minor
 * variations in the matrix caused by the non-0.0 values obtained for the sine and cosine. If these
 * transforms are not classified as quadrant rotations then subsequent code which attempts to
 * optimize further operations based upon the type of the transform will be relegated to its most
 * general implementation.
 * <p>
 * Because quadrant rotations are fairly common, this class should handle these cases reasonably
 * quickly, both in applying the rotations to the transform and in applying the resulting transform
 * to the coordinates. To facilitate this optimal handling, the methods which take an angle of
 * rotation measured in radians attempt to detect angles that are intended to be quadrant rotations
 * and treat them as such. These methods therefore treat an angle <em>theta</em> as a quadrant
 * rotation if either <code>Math.sin(<em>theta</em>)</code> or <code>Math.cos(<em>theta</em>)</code>
 * returns exactly 1.0 or -1.0. As a rule of thumb, this property holds true for a range of
 * approximately 0.0000000211 radians (or 0.00000121 degrees) around small multiples of
...
public class AffineTransform implements Cloneable, java.io.Serializable {
  ...
  /*
   * For methods which combine together the state of two separate transforms and dispatch based upon
   * the combination, these constants specify how far to shift one of the states so that the two
   * states are mutually non-interfering and provide constants for testing the bits of the shifted
   * (HI) state. The methods in this class use the convention that the state of "this" transform is
   * unshifted and the state of the "other" or "argument" transform is shifted (HI).
   */
  ...
  /**
   * Constructs a new <code>AffineTransform</code> from an array of floating point values
   * representing either the 4 non-translation enries or the 6 specifiable entries of the 3x3
   ...
   *          <code>AffineTransform</code> object. The length of the array is assumed to be at least
   *          4. If the length of the array is less than 6, only the first 4 values are taken. If
  ...
  /**
   * Constructs a new <code>AffineTransform</code> from 6 double precision values representing the 6
   * specifiable entries of the 3x3 transformation matrix.
   * 
   * @param m00 the X coordinate scaling element of the 3x3 matrix
   * @param m10 the Y coordinate shearing element of the 3x3 matrix
  ...
  /**
   * Constructs a new <code>AffineTransform</code> from an array of double precision values
   ...
   * @return an <code>AffineTransform</code> object that represents a translation transformation,
   ...
   * </pre>
   * 
   * Rotating by a positive angle theta rotates points on the positive X axis toward the positive Y
   * axis. Note also the discussion of <a href="#quadrantapproximation">Handling 90-Degree
   * Rotations</a> above.
   * 
   * @param theta the angle of rotation measured in radians
   * @return an <code>AffineTransform</code> object that is a rotation transformation, created with
   ...
   * equivalent to translating the coordinates so that the anchor point is at the origin (S1), then
   * rotating them about the new origin (S2), and finally translating so that the intermediate
   * origin is restored to the coordinates of the original anchor point (S3).
   * <p>
   ...
   * @return an <code>AffineTransform</code> object that rotates coordinates around the specified
   *         point by the specified angle of rotation.
   * @since 1.2
   */
   ...
   * origin to the specified vector coordinates. If both <code>vecx</code> and <code>vecy</code> are
   * 0.0, an identity transform is returned. This operation is equivalent to calling:
   * 
   * <pre>
   ...
   * @return an <code>AffineTransform</code> object that rotates coordinates according to the
   ...
   * @return an <code>AffineTransform</code> object that rotates coordinates by the specified number
   *         of quadrants.
   * @since 1.6
   */
   ...
   * @return an <code>AffineTransform</code> object that scales coordinates by the specified
   *         factors.
   * @since 1.2
   */
   ...
   * @return an <code>AffineTransform</code> object that shears coordinates by the specified
   *         multipliers.
   * @since 1.2
   */
   ...
   * of the appriopriate flag bits. A valid combination of flag bits is an exclusive OR operation
   ...
   * <p>
   * If the determinant is non-zero, then this transform is invertible and the various methods that
   * depend on the inverse transform do not need to throw a {@link NoninvertibleTransformException}.
   * If the determinant is zero then this transform can not be inverted since the transform maps all
   * input coordinates onto a line or a point. If the determinant is near enough to zero then
   * inverse transform operations might not carry enough precision to produce meaningful results.
   * <p>
   * If this transform represents a uniform scale, as indicated by the <code>getType</code> method
   * then the determinant also represents the square of the uniform scale factor by which all of the
   * points are expanded from or contracted towards the origin. If this transform represents a
...
//    throw new InternalError("missing case in transform state switch");
   ...
   * concatenate(R), where R is an <code>AffineTransform</code> represented by the following matrix:
   * 
   * <pre>
   *          [   cos(theta)    -sin(theta)    0   ]
   *          [   sin(theta)     cos(theta)    0   ]
   ...
   * This operation is equivalent to translating the coordinates so that the anchor point is at the
   * origin (S1), then rotating them about the new origin (S2), and finally translating so that the
   ...
   * such that coordinates along the former positive X axis will subsequently align with the vector
   * pointing from the origin to the specified vector coordinates. If both <code>vecx</code> and
   * <code>vecy</code> are 0.0, no additional rotation is added to this transform. This operation is
   ...
   * translating the coordinates so that the anchor point is at the origin (S1), then rotating them
   * about the new origin (S2), and finally translating so that the intermediate origin is restored
   ...
   * rotation is such that coordinates along the former positive X axis will subsequently align with
   * the vector pointing from the origin to the specified vector coordinates. If both
   * <code>vecx</code> and <code>vecy</code> are 0.0, the transform is set to an identity transform.
  ...
  /**
   * Sets this transform to a copy of the transform in the specified <code>AffineTransform</code>
   * object.
   * 
   ...
   * <code>AffineTransform</code> Cx in the most commonly useful way to provide a new user space
   * that is mapped to the former user space by <code>Tx</code>. Cx is updated to perform the
   * combined transformation. Transforming a point p by the updated transform Cx' is equivalent to
   * first transforming p by <code>Tx</code> and then transforming the result by the original
   * transform Cx like this: Cx'(p) = Cx(Tx(p)) In matrix notation, if this transform Cx is
   * represented by the matrix [this] and <code>Tx</code> is represented by the matrix [Tx] then
   ...
   * </pre>
   * 
   * @param Tx the <code>AffineTransform</code> object to be concatenated with this
   *          <code>AffineTransform</code> object.
   * @see #preConcatenate
   * @since 1.2
   */
  ...
  /**
   * Concatenates an <code>AffineTransform</code> <code>Tx</code> to this
   * <code>AffineTransform</code> Cx in a less commonly used way such that <code>Tx</code> modifies
   * the coordinate transformation relative to the absolute pixel space rather than relative to the
   * existing user space. Cx is updated to perform the combined transformation. Transforming a point
   * p by the updated transform Cx' is equivalent to first transforming p by the original transform
   * Cx and then transforming the result by <code>Tx</code> like this: Cx'(p) = Tx(Cx(p)) In matrix
   ...
   * <p>
   * If this transform maps all coordinates onto a point or a line then it will not have an inverse,
   * since coordinates that do not lie on the destination point or line will not have an inverse
   * mapping. The <code>getDeterminant</code> method can be used to determine if this transform has
   * no inverse, in which case an exception will be thrown if the <code>createInverse</code> method
   * is called.
   * 
  ...
  public AffineTransform createInverse() throws NoninvertibleTransformException {
    ...
    switch (state){
      ...
      case (APPLY_SHEAR | APPLY_SCALE | APPLY_TRANSLATE) :
        ...
        return new AffineTransform(m11 / det, -m10 / det, -m01 / det, m00 / det, (m01 * m12 - m11 * m02) / det, (m10
            * m02 - m00 * m12)
   ...
   * since coordinates that do not lie on the destination point or line will not have an inverse
   * mapping. The <code>getDeterminant</code> method can be used to determine if this transform has
   * no inverse, in which case an exception will be thrown if the <code>invert</code> method is
   * called.
   * 
   * @see #getDeterminant
   * @exception NoninvertibleTransformException if the matrix cannot be inverted.
  ...
  /**
   * Transforms the specified <code>ptSrc</code> and stores the result in <code>ptDst</code>. If
   * <code>ptDst</code> is <code>null</code>, a new {@link Point2D} object is allocated and then the
   * result of the transformation is stored in this object. In either case, <code>ptDst</code>,
   * which contains the transformed point, is returned for convenience. If <code>ptSrc</code> and
   * <code>ptDst</code> are the same object, the input point is correctly overwritten with the
   * transformed point.
   * 
   * @param ptSrc the specified <code>Point2D</code> to be transformed
   * @param ptDst the specified <code>Point2D</code> that stores the result of transforming
   *          <code>ptSrc</code>
  ...
  /**
   * Transforms an array of point objects by this transform. If any element of the
   * <code>ptDst</code> array is <code>null</code>, a new <code>Point2D</code> object is allocated
   * and stored into that element before storing the results of the transformation.
   * <p>
   * Note that this method does not take any precautions to avoid problems caused by storing results
   * into <code>Point2D</code> objects that will be used as the source for calculations further down
   * the source array. This method does guarantee that if a specified <code>Point2D</code> object is
   * both the source and destination for the same single point transform operation then the results
   * will not be stored until the calculations are complete to avoid storing the results on top of
   * the operands. If, however, the destination <code>Point2D</code> object for one operation is the
   * same object as the source <code>Point2D</code> object for another operation further down the
   * source array then the original coordinates in that point are overwritten before they can be
   * converted.
   * 
   ...
   * @param dstOff the offset to the location of the first transformed point object that is stored
   ...
   * 
   * @param srcPts the array containing the source point coordinates. Each point is stored as a pair
   *          of x,&nbsp;y coordinates.
   * @param dstPts the array into which the transformed point coordinates are returned. Each point
  ...
  /**
   * Transforms an array of double precision coordinates by this transform. The two coordinate array
   * sections can be exactly the same or can be overlapping sections of the same array without
   * affecting the validity of the results. This method ensures that no source coordinates are
   * overwritten by a previous operation before they can be transformed. The coordinates are stored
   * in the arrays starting at the indicated offset in the order
   * <code>[x0, y0, x1, y1, ..., xn, yn]</code>.
   * 
   * @param srcPts the array containing the source point coordinates. Each point is stored as a pair
   *          of x,&nbsp;y coordinates.
  ...
  /**
   * Transforms an array of floating point coordinates by this transform and stores the results into
   * an array of doubles. The coordinates are stored in the arrays starting at the specified offset
  ...
  /**
   * Transforms an array of double precision coordinates by this transform and stores the results
   * into an array of floats. The coordinates are stored in the arrays starting at the specified
   ...
   * . If <code>ptDst</code> is <code>null</code>, a new <code>Point2D</code> object is allocated
   * and then the result of the transform is stored in this object. In either case,
   * <code>ptDst</code>, which contains the transformed point, is returned for convenience. If
   * <code>ptSrc</code> and <code>ptDst</code> are the same object, the input point is correctly
   ...
   * coordinates are stored in the arrays starting at the specified offset in the order
   * <code>[x0, y0, x1, y1, ..., xn, yn]</code>.
   * 
   * @param srcPts the array containing the source point coordinates. Each point is stored as a pair
   *          of x,&nbsp;y coordinates.
  ...
  /**
   * Transforms the relative distance vector specified by <code>ptSrc</code> and stores the result
   * in <code>ptDst</code>. A relative distance vector is transformed without applying the
   * translation components of the affine transformation matrix using the following equations:
   * 
   * <pre>
   ...
   * </pre>
   * 
   * If <code>ptDst</code> is <code>null</code>, a new <code>Point2D</code> object is allocated and
   * then the result of the transform is stored in this object. In either case, <code>ptDst</code>,
   * which contains the transformed point, is returned for convenience. If <code>ptSrc</code> and
   * <code>ptDst</code> are the same object, the input point is correctly overwritten with the
   * transformed point.
   * 
   ...
   * coordinates are overwritten by a previous operation before they can be transformed. The
   * coordinates are stored in the arrays starting at the indicated offset in the order
   * <code>[x0, y0, x1, y1, ..., xn, yn]</code>.
   * 
   * @param srcPts the array containing the source distance vectors. Each vector is stored as a pair
   *          of relative x,&nbsp;y coordinates.
   * @param dstPts the array into which the transformed distance vectors are returned. Each vector
   ...
   * @param dstOff the offset to the location of the first transformed vector that is stored in the
   *          destination array
   ...
   * @return a new <code>Shape</code> object that defines the geometry of the transformed
   *         <code>Shape</code>, or null if {@code pSrc} is null.
   * @since 1.2
   */
   ...
   * 
   * @return <code>true</code> if this <code>AffineTransform</code> is an identity transform;
   ...
   * @return <code>true</code> if <code>obj</code> equals this <code>AffineTransform</code> object;
   *         <code>false</code> otherwise.
   * @since 1.2
   */
  ...
  /*
   * Serialization support. A readObject method is neccessary because the state field is part of the
   * implementation of this particular AffineTransform and not part of the public specification. The
   * state variable's value needs to be recalculated on the fly by the readObject method as it is in
   * the 6-argument matrix constructor.
   */
